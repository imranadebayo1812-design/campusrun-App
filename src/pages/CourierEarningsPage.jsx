    ...
    <div className="bg-surface-950 min-h-full">
          ...
          <div className="bg-surface-900 border border-white/[0.08] rounded-2xl p-4 space-y-3">
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                ...
                className="flex-1 bg-green-600 text-white font-semibold py-2.5 rounded-xl text-sm"
              >
                Submit
              </button>
            </div>
      ...
      <div className="px-4">
        <p className="text-sm font-semibold text-white mb-3 flex items-center gap-1.5">
          <TrendingUp className="w-4 h-4 text-brand-400" /> Completed Deliveries
        </p>
        {deliveries.length === 0 ? (
              ...
              <div key={d.id} className="bg-surface-900 border border-white/[0.08] rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-green-400">
                    +₦{((d.delivery_fee || 0) + (d.tip || 0)).toLocaleString()}
                  </p>
                  {d.tip > 0 && <p className="text-xs text-gray-500">+₦{d.tip} tip</p>}
                </div>
              </div>
            ))}
          </div>
        ...
        {withdrawals.length > 0 && (
          <div className="mb-5">
            <p className="text-sm font-semibold text-white mb-3 flex items-center gap-1.5">
              <Clock className="w-4 h-4 text-gray-400" /> Withdrawal History
            </p>
            <div className="space-y-2">
              {withdrawals.map(w => (
                <div key={w.id} className="bg-surface-900 border border-white/[0.08] rounded-xl p-3 flex justify-between items-center">
                  <div>
                    <p className="text-sm font-medium text-white">{w.bank_name} — {w.account_number}</p>
                    <p className="text-xs text-gray-500">

...
import { MOCK_REFERRAL } from '@/lib/mockData';
...
export default function ReferralPage() {
  ...
  function shareLink() {
    const text = `Join CampusRun with my referral code ${ref.code} and get fast campus deliveries at Nile University! https://campusrun.online/ref/${ref.code}`;
    if (navigator.share) {
      navigator.share({ title: 'CampusRun Referral', text }).catch(() => {});
    } else {
      navigator.clipboard.writeText(text).catch(() => {});
    }
  }

  const canWithdraw = ref.available >= 1000;
  const needed = Math.max(0, 1000 - ref.available);
  ...
  return (
    <div className="bg-surface-950 min-h-full">
      ...
      <div className="mx-4 mb-5">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4 relative z-10">
            <p className="text-4xl font-black text-white tracking-widest">{ref.code}</p>
            <button
              onClick={copyCode}
      ...
      <div className="mx-4 mb-5 grid grid-cols-3 gap-3">
        {[
          { icon: Users, label: 'Referrals', value: ref.referrals, color: 'text-brand-400' },
          { icon: Banknote, label: 'Total Earned', value: `₦${ref.total_earned.toLocaleString()}`, color: 'text-green-400' },
          { icon: Wallet, label: 'Available', value: `₦${ref.available.toLocaleString()}`, color: 'text-yellow-400' },
      ...
      <div className="mx-4 mb-5 bg-surface-900 border border-white/[0.08] rounded-2xl overflow-hidden">
        ...
        {[
          ...
          { step: '3', text: 'You earn ₦100 instantly in your referral balance' },
        ].map(({ step, text }) => (
      ...
      {!canWithdraw && (
        <div className="mx-4 mb-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl px-4 py-3.5">
          ...
          <p className="text-xs text-amber-400/70 mt-0.5">Minimum ₦1,000 required to withdraw referral earnings.</p>
        </div>
      )}
